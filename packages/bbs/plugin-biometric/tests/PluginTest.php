...
/**
 * Plugin validation tests for BiometricAndroid.
 *
...
describe('Plugin Manifest', function () {
    ...
    it('has required fields', function () {
        ...
        expect($manifest['name'])->toBe('bbs/plugin-biometric-android');
        expect($manifest['namespace'])->toBe('BiometricAndroid');
    });
...
describe('Native Code', function () {
    it('has Android Kotlin file', function () {
        $kotlinFile = $this->pluginPath . '/resources/android/BiometricAndroidFunctions.kt';

        expect(file_exists($kotlinFile))->toBeTrue();

        $content = file_get_contents($kotlinFile);
        expect($content)->toContain('package com.bbs.plugins.biometric_android');
        expect($content)->toContain('object BiometricAndroidFunctions');
        expect($content)->toContain('BridgeFunction');
    });

    it('has iOS Swift file', function () {
        $swiftFile = $this->pluginPath . '/resources/ios/BiometricAndroidFunctions.swift';

        expect(file_exists($swiftFile))->toBeTrue();

        $content = file_get_contents($swiftFile);
        expect($content)->toContain('enum BiometricAndroidFunctions');
        expect($content)->toContain('BridgeFunction');
    });
...
describe('PHP Classes', function () {
    it('has service provider', function () {
        $file = $this->pluginPath . '/src/BiometricAndroidServiceProvider.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Bbs\BiometricAndroid');
        expect($content)->toContain('class BiometricAndroidServiceProvider');
    });

    it('has facade', function () {
        $file = $this->pluginPath . '/src/Facades/BiometricAndroid.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Bbs\BiometricAndroid\Facades');
        expect($content)->toContain('class BiometricAndroid extends Facade');
    });

    it('has main implementation class', function () {
        $file = $this->pluginPath . '/src/BiometricAndroid.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Bbs\BiometricAndroid');
        expect($content)->toContain('class BiometricAndroid');
    });
});
